...
public sealed class FromTsvCommand : ShellCommand
{
    public FromTsvCommand()
        : base("from-tsv", "Parses TSV text into shell row objects.", "from-tsv [tsv-text]") { }
    ...
    public override async IAsyncEnumerable<object?> ExecuteAsync(CommandContext context)
    {
        ...
        var text = await StructuredTextInput.ReadAllTextAsync(
            context,
            parsed.Positionals,
            "from-tsv expects TSV text from the pipeline or an explicit argument.");

        IReadOnlyList<string[]> records;

        try
        {
            records = TsvParser.Parse(text);
        }
        catch (Exception exception) when (exception is InvalidOperationException)
        {
            throw new InvalidOperationException($"Could not parse TSV input. {exception.Message}");
        }
