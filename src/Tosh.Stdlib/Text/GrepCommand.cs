...
[CommandOption("-r, -R, --recursive", "Recurse into directories when path arguments are supplied.")]
[CommandOption("-w, --word-regexp", "Match only whole words.")]
[CommandOption("-o, --only-matching", "Print only the matching portion of each line.")]
[CommandOption("-c, --count", "Print only the count of matching lines per source.")]
[CommandOption("-l, --files-with-matches", "Print only the names of files that contain a match.")]
[CommandOption("-L, --files-without-match", "Print only the names of files that contain no match.")]
[CommandOption("-A, --after-context", "Print N lines of trailing context after each match.", ValueClrType = typeof(int))]
[CommandOption("-B, --before-context", "Print N lines of leading context before each match.", ValueClrType = typeof(int))]
[CommandOption("-C, --context", "Print N lines of context around each match.", ValueClrType = typeof(int))]
