...
[CommandOption("-maxdepth <n>", "Limit recursion to at most N directory levels below the starting path.")]
[CommandOption("-mindepth <n>", "Skip results shallower than N directory levels.")]
[CommandOption("-empty", "Match only empty files and empty directories.")]
[CommandOption("-size <expr>", "Filter by size; accepts +N/-N suffixes c/k/M/G (e.g. -size +1M).")]
[CommandOption("-mtime <expr>", "Filter by modification age in days (+N for older, -N for newer).")]
[CommandOption("-days <expr>", "Alias for -mtime.")]
[CommandOption("-newer-than <path-or-time>", "Match entries modified after the given file or timestamp.")]
[CommandOption("-older-than <path-or-time>", "Match entries modified before the given file or timestamp.")]
...
[CommandOutput("Returns typed filesystem entries with rich metadata that flow naturally through the object pipeline.", TypeName = "FileSystemEntry", Members = "Name, FullName, Type, Size, Modified", ClrType = typeof(IAsyncEnumerable<FileSystemEntry>))]
