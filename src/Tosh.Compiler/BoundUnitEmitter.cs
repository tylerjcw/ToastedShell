...
internal sealed class EmitterImpl
{
    ...
    public void Run()
    {
        ...
        if (ProgramUsesBlockExpressions() || ProgramHasTypeDefinitions() || ProgramHasModuleDefinitions() || hasSubcommandDispatch)
        {
        ...
        // Register every user-defined type (class / record / struct /
        // enum / union / interface / trait) by replaying its source
        // slice through the engine. Done up-front so any later `new`
        // expressions or member accesses can resolve the type.
        foreach (var statement in _unit.Root.Statements)
        {
            if (IsTypeDefinitionStatement(statement, out var span))
            {
                // Source-replay still runs even when a CLR shell was
                // emitted (the engine needs the type for dynamic
                // `new` / member-access dispatch). But for
                // shell-emittable shapes, the replay is a no-op-
                // equivalent re-build of a type the assembly already
                // describes through `[ToshType]`, so we don't fire
                // the Tier 3 diagnostic — same precedent as pure
                // module bodies.
                if (!IsClrShellEmittedTypeDefinition(statement))
                {
        ...
        // Register every top-level module by replaying its source
        // slice. Nested modules are produced as a side-effect of
        // executing the outer module's body. Each top-level module
        // also emits a [ToshModule] assembly attribute so external
        // tooling can enumerate compiled modules via reflection.
        foreach (var statement in _unit.Root.Statements)
        {
            if (statement is BoundModuleDefinition mod)
            {
    ...
    /// <summary>
    /// Returns true if the bound unit contains any user-defined
    /// type-declaration statement (class / record / struct / enum /
    /// union / interface / trait). Used to decide whether the
    /// source-registration prologue should run even when the program
    /// has no block expressions.
    /// </summary>
    private bool ProgramHasTypeDefinitions()
    {
        foreach (var stmt in _unit.Root.Statements)
        {
            if (IsTypeDefinitionStatement(stmt, out _)) return true;
        }
        return false;
    }

    /// <summary>
    /// True if the bound unit contains any top-level
    /// <c>module</c> declaration. Forces the source-registration
    /// prologue on so module bodies can be replayed via
    /// <c>ToshHost.RegisterModuleFromSource</c>.
    /// </summary>
    private bool ProgramHasModuleDefinitions()
    {
        foreach (var stmt in _unit.Root.Statements)
        {
            if (stmt is BoundModuleDefinition) return true;
        }
        return false;
    }
        ...
        else
        {
            ...
            foreach (var capture in block.Captures)
            {
                ...
                if (_typedParamLocals.TryGetValue(capture, out var typedParamLocal))
                {
                    ...
                    // Already object-boxed at method entry; no further boxing.
                }
