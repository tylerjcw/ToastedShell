...
| Spread elements (`[...$xs]`), record `{| name: ... |}`, slice indices                | done        | Spread elements: list literals call `ToshHost.SpreadArgs` for `...expr` items. Record literals accept `:` and `=` separators. Slice `$xs[1..3]`: `ShellIndexingUtilities.GetSlice` materialises `ToshRange.Enumerate()` into the target collection (string / array / IList / IEnumerable); compiler emits `ToshRange` values via `BoundUnitEmitter.EmitRange`. |
