...
**Last updated:** 2026-08-13 — Type-system semantics landed during the July/August stabilization round are now documented. New **§Type Annotations** (Type System) states what satisfies an annotation — a subclass satisfies its base in every annotated position, an interface or trait names a contract that must be *declared* rather than merely matched structurally — and where an annotation's name is resolved, which is the scope that declares it rather than the scope executing when the value arrives (`TS-P2-99`, `TS-P2-106`, `TS-P2-107`). New **§Type Narrowing** (Control Flow) documents that `if ($x is T)` and a `_ is T` match arm narrow identically (`TS-P2-108`). **§Variable Declarations** gains inferred-type reassignment, including that an annotated variable does not re-infer and a branch reassignment does not leak (`TS-P2-87`). The operator precedence table moves `as` from level 7 to level 1 — it is a cast and binds tighter than every binary operator, while `is` stays loose (`TS-P2-105`). The native-interop chapter states what a checked call with *zero* out parameters yields, which was the one case it never covered (`TS-P2-88`). Cookbook examples are now executed by `SpecWorkedExampleTests`, harvested from this file rather than curated, so a command that does not exist fails the suite (`TS-P2-26`).

**Previously:** 2026-05-06 —
---
