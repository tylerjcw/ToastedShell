...
\note{Quantities enforce dimensional analysis: compatible dimensions add/compare cleanly; incompatible ones raise an error.}
...
\item \texttt{date add/sub} accepts a \texttt{TemporalAmount}, so both \texttt{2d} and \texttt{1mo} work.
\item \texttt{sum} and \texttt{avg} handle numeric, \texttt{StorageSize}, and \texttt{TimeSpan} streams directly.
\end{itemize}
...
\note{Aggregation is strongest on shell-native temporal and size types. Quantities are best used for direct formula-style calculations and comparisons.}
