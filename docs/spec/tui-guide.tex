...
\begin{lstlisting}
{| Path = "~/notes.md", Mode = "file", OnSubmit = &Save |}
{| Path = $target, From = "/etc", Mode = "directory", OnChange = &Preview |}
\end{lstlisting}

A path you can type, or browse for. Typing edits it, \code{F2} opens a filesystem
browser, and choosing there puts the chosen path back into the text --- browse when
you do not know it, type when you do. \code{Enter} settles the path and \code{Esc}
abandons the edit; \code{Esc} \emph{while browsing} only closes the browser, and the
text is still there.

The browse key is \code{F2} rather than a letter because it is checked before the
text input, so whatever it is cannot be typed into the path. A path field that
cannot spell \code{/usr/bin} is not a path field. The config browser's own editor
uses \code{b} and offers a separate raw-text editor for the paths that contain one.

\code{From} is where browsing starts when it should not be the path already typed;
\code{Mode} is \code{"file"}, \code{"directory"} or \code{"any"}.

\subsection{\texttt{FilePicker}}

\begin{lstlisting}
{| FilePicker = "~/projects", Mode = "file", OnSelect = &Open |}
{| FilePicker = ".", Mode = "directory", OnSelect = &Chdir, OnCancel = &Stay |}
\end{lstlisting}

A filesystem browser that answers with the path the reader chose. Up and Down move
through the entries, Right enters a directory, Left or \code{Backspace} goes to the
parent, \code{Enter} opens a directory or chooses a file, and \code{Space} chooses
the highlighted path outright. \code{Esc} abandons the browse.

\code{Mode} is \code{"file"}, \code{"directory"} or \code{"any"}, and decides what
may be chosen rather than what is shown --- a file picker still walks directories.
\code{Start} puts the cursor on a path within the starting directory.

Choosing or abandoning ends the screen, as \code{Confirm} does and unlike the
editors: a picker is asked a question and answers it, and one that stayed up
afterwards would leave the reader browsing a filesystem they had finished with.
\code{Exit = false} keeps it embedded.

\subsection{\texttt{Arrange}}

\begin{lstlisting}
{| Arrange = ["Time", "Directory", "Git", "Exit code"],
   Minimum = 1, OnCommit = &SaveLayout |}
\end{lstlisting}

Which of these to use, and in what order. Each entry carries a box --- \code{[x]}
for in, \code{[ ]} for out --- and the arrows move the cursor while \code{Shift}
with them moves the \emph{entry}. \code{Space} toggles one, \code{Enter} settles
the arrangement and \code{Esc} abandons it.

\code{Minimum} is how few entries may be left in. A toggle that would go below it
is refused rather than applied, and \code{OnRefuse} is told which entry was asked
about --- a prompt with nothing in it is not a prompt.

\code{OnChange} and \code{OnCommit} are both given the included entries in their
current order, which is the answer the caller wanted; the entries handed in are the
entries handed back, so a list of strings stays a list of strings.
