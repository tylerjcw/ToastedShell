...
Sixel, for the terminals that speak that and not this, is still \code{TUI-0025}.
...
\code{"shift+tab"}, \code{"f5"}, \code{"/"}. \code{"space"}, \code{"esc"},
\code{"up"}, \code{"pgdn"} and their like are accepted alongside the enum's own
names --- and \code{"space"} is the only way to say it, because a chord is trimmed
before it is read and a literal \code{" "} does not survive that.
