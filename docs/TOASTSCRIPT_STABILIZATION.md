...
| `TS-P3-13` | Planned — filed 2026-08-08 | The stabilization tooling does not escape `\|` when filing an item, so any description quoting a pipeline breaks its own table row. `TS-P2-47` truncated mid-sentence at `$c \| members` and could not be read back when re-probed. | `scripts/file-item.tosh` escapes pipes in the description cell, and a check reports any row whose cell count differs from the table header. |
