...
| `TS-P2-85` | Planned — filed 2026-08-12 | A computed property does not work on a struct at all. `struct S { prop Y => 7 }` then `(new S()).Y` reports "Member Y was not found on type S" — `CreateInstance` skips `IsComputed` properties when seeding stored values, which is correct, and nothing evaluates them on access, which is not. `GetMembers` lists them, so introspection advertises a member that reading refuses. Identical on the installed binary, so it predates the `TS-P2-83` work that found it; the class form works. | A computed property on a struct is evaluated on access, as it is on a class, and `$s \| members` and `$s.Y` agree about what exists; a corpus covers a constant body, one reading `$this`, and one reading a stored field, beside the class equivalents. |
| `TS-P2-86` | Planned — filed 2026-08-12 | `call` and `race` reject argument counts their own usage strings document, at runtime as well as at check time. `which call` reports "call <method-name> [args...] or call <type-name> <method-name> [args...]" while `(new System.Random \| call Next 1 100)` — as written in `examples/showcase.tosh` — fails with "Command call expects between 0 and 2 argument(s) but received 3". `which race` reports "race <callable\|block> <callable\|block> [...]" while `race { … } { … }`, as written in `examples/showcase2.tosh`, fails with "expects 1 argument(s) but received 2". Both example scripts are therefore broken as shipped. Found by the `TS-P2-84` sweep, which correctly reported them; the declared arity metadata is what is wrong. | A commands declared arity matches what it accepts, so a variadic command declares itself variadic; `call` and `race` run as their usage strings and the shipped examples describe them, and a corpus executes both example lines so the metadata cannot drift from the behaviour again. |
...
| `TS-P2-112` | Planned — filed 2026-08-13 | **A command call in an `if` condition needs a second pair of parentheses, and the diagnostic blames the wrong thing.** `if (is-dir $path) { … }` reports `tosh.parser.missing_closing_parenthesis` with the label "this condition never closes", pointing at the condition rather than at the call inside it. `if ((is-dir $path))` works, and so does `echo (is-dir $path)`. The model is consistent — an `if` condition is an expression, and a parenthesised command call is how a call *becomes* an expression, so the outer parentheses are `if`'s syntax and the inner ones do the conversion — but `if (some-command $x)` is what any shell user writes first, and nothing in the diagnostic suggests the real cause. Confirmed general: a user-declared function fails the same way. Found by `TS-P2-26`'s corpus, in the specification's own "Interactive File Browser" example. | Either a bare command call is accepted in condition position, or the diagnostic names the call as the cause and suggests the parenthesised form. Same family as `TS-P2-105`, where a misleading diagnostic pointed at the operator rather than the cast. |
| `TS-P2-113` | Planned — filed 2026-08-13 | **An array holding an empty array collapses when piped or iterated.** `var r = [[]]` has `$r.Length` 1, but `$r \| count` is **0** — the pipeline flattens the outer array and the empty inner one contributes nothing, so the single element vanishes. `$r \| map …` yields 1, so `count` and `map` do not even agree with each other about how many items they were given. The consequence is that a fold seeded with the empty set cannot be written: `power-set` builds `[[]]` and grows it, and every spelling attempted — `reduce` with a block, `reduce` with an explicit lambda, and an explicit `for` loop — produced null or an empty result. Found while trying to make the specification's "Power Set" example run for `TS-P2-26`; the parse error there is fixed and filed separately from this, which is a semantic defect underneath it. | `[[]]` is a one-element sequence everywhere: `count`, `map`, `for` and concatenation all agree with `.Length`. A corpus covers an empty inner array, a mix of empty and non-empty inner arrays, and the power-set fold that motivated it. |
| `TS-P2-111` | Planned — filed 2026-08-13 | **A `Double` cannot be cast to an integer type at all.** `7.9 as int` reports `Cannot convert 'Double' to 'int'`, and so do `as long` and `as Int32`; `7.9 as decimal` succeeds, and `"7.9" as int` fails the same way from `String`. Truncating a double to an integer is the most common cast there is, and the only spellings that work are a `Math` call. Note the near-miss that makes this easy to misread: `(10 / 4) as int` *succeeds* and yields 2, which looks like truncation but is not — `/` is integer division on two ints, so the value was already `Int32` and the cast was an identity. Found while verifying `TS-P2-105`, and independent of it: it reproduces on the parenthesised form and on the pre-fix build. | `as` performs the narrowing conversions its name implies — `Double`/`Decimal` to the integer types, and a numeric `String` to a number — or reports a diagnostic that says truncation is required and names the call to make. A corpus covers each numeric pair in both directions, including the lossy ones, and states which are permitted. |
| `TS-P2-110` | Planned — filed 2026-08-13 | **`--profile-startup` silently does nothing for any invocation except the REPL.** `CliInvocationResolver.cs:294` passes `ProfileStartup` only when building a `CliInvocationKind.Repl` plan, so `tosh --profile-startup -c "…"` and `tosh --profile-startup script.tosh` accept the flag, print no timing, and exit successfully. The REPL form works and is genuinely useful — it is what localised a 628 ms profile load during this session — but `-c` is the invocation a startup investigation most wants to measure, because it is the one a prompt hook or a script pays on every call. | `--profile-startup` reports the same phase breakdown for `-c`, for a script path, and for the REPL. A corpus asserts the breakdown is emitted for each invocation kind, and that the flag is rejected with a diagnostic rather than silently ignored wherever it genuinely cannot apply. |
