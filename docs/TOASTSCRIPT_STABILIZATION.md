...
**In progress** (as of July 30, 2026 — 97 items, 53 complete; P0 8/8,
P1 23/36, P2 20/42, P3 2/11; plus one partially complete and one
withdrawn. Suite 3,630 passing, 1 skipped. Counted from the tables
...
| `TS-P2-43` | Planned — filed 2026-07-30 | **Two directories under `src/` are not in the build.** `src/Tosh.Core` (879 lines, last touched 2026-04-29) has no `.csproj` at all — `ARCHITECTURE.md` described it as a "legacy shim being phased out" and the phase-out finished in effect but never in the tree. `src/Tosh.Dap` (514 lines, last touched 2026-04-30) has a `.csproj` that no solution includes, so a Debug Adapter Protocol server exists but is never compiled, never tested, and cannot have been used since April. Neither is referenced by any project. The only external mention is the string `"Tosh.Core.dll"` in `ToshPublisher.RuntimeDependencyFileNames`, which is harmless — the copy loop is `File.Exists`-guarded. | Decide per directory and act: delete `Tosh.Core` (git history keeps it) and drop the stale publish entry; for `Tosh.Dap`, either add it to `Tosh.slnx` with a smoke test alongside the other protocol surfaces, or delete it and file DAP support as future work. Dormant code that looks live is worse than either. |
