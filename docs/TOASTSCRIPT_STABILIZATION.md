...
| `TS-P3-24` | Proposed — filed 2026-08-16 | Variadic parameter syntax for native bindings | `func f(count: int, ...)` is rejected with *Native interop does not currently support '...'*. The capability is **not** missing: declaring the concrete arity works, and was verified against both a purpose-built `sum_varargs` and libc's `snprintf`, including a `double` argument (`"f=%.2f", 3.5` → `f=3.50`), so the x86-64 SysV register-count rule is being satisfied. That is also exactly what C# P/Invoke requires — one declaration per arity. What is missing is the convenience Python's `ctypes` and Nim's `varargs` provide: one declaration usable at any arity. Worth doing only if a real binding needs many arities of the same function; the workaround is a line of code, and the current diagnostic names the limit clearly rather than failing obscurely. |
