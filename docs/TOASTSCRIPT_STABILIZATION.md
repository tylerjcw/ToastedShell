...
| `TS-P3-25` | Proposed — filed 2026-08-16 | Native failure diagnostics carry an unstable code | A failing `-> ok` binding reports code `tosh.native./tmp/…/libprobe.so.may_fail` — the **full filesystem path of the library** embedded in the diagnostic code. Codes are the identifier `hush` matches on and the diagnostic reference documents, so one containing an absolute path cannot be hushed portably, cannot be documented, and differs between machines for the same failure. Separately, *Native library 'X' was not found* and *Unable to find an entry point named 'Y'* both report the generic `tosh.runtime.error` rather than codes of their own, so the two most common binding mistakes cannot be targeted at all. Found during the native interop audit. \| Give the native call failures real codes — something like `tosh.native.call_failed`, `tosh.native.library_not_found`, `tosh.native.entry_point_not_found` — and move the library path and symbol name into the message and label, where they belong and already partly are. |
