...
/// <summary>
/// <c>sort --ordinal</c> compares by code point — <c>TS-P2-75</c>.
/// </summary>
/// <remarks>
/// <para>
/// <c>sort</c> compares with <see cref="StringComparer.OrdinalIgnoreCase"/>, which is the
/// right default for a shell — <c>Apple</c> and <c>apple</c> belong next to each other —
/// and has a consequence that surprises. Case folding raises lowercase letters above
/// <c>_</c>: uppercasing turns <c>s</c> (0x73) into <c>S</c> (0x53), which sorts *below*
/// <c>_</c> (0x5F), so <c>expected_record_fields</c> comes before
/// <c>expected_record_field_default</c> while by code point it comes after. Anything
/// generated and committed wants the code-point order, and there was no way to ask.
/// </para>
/// <para>
...
public sealed class SortOrdinalTests
{
    ...
    {
        // Case-insensitive by default, which is deliberate and stays.
        var results = await RunAsync(Underscored + "\n$xs | sort");
    ...
    [Fact]
    public async Task Case_folding_is_what_the_flag_turns_off()
    {
        var insensitive = await RunAsync("""["b", "A", "a", "B"] | sort""");
        var ordinal = await RunAsync("""["b", "A", "a", "B"] | sort -o""");

        // Ordinal puts every capital before every lowercase; the default interleaves.
        Assert.Equal(["A", "B", "a", "b"], ordinal);
        Assert.NotEqual(ordinal, insensitive);
    }

    [Fact]
    public async Task Unique_agrees_with_the_comparison_it_follows()
    {
        // `-u` deduplicates with the same case sensitivity as the sort, or `-u -o` would
        // fold `Alpha` and `alpha` together right after placing them apart.
        Assert.Equal(["Alpha", "beta"], await RunAsync("""["Alpha", "alpha", "beta"] | sort -u"""));
        Assert.Equal(["Alpha", "alpha", "beta"], await RunAsync("""["Alpha", "alpha", "beta"] | sort -u -o"""));
    }
