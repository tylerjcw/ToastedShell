...
{
    [Fact]
    public async Task Set_alias_and_select_pick_projection_commands_work()
    {
        var engine = new ToshEngine(ToshRuntime.CreateDefault());

        var variableResults = await engine.ExecuteToListAsync("set greeting = \"hello\"\necho $greeting");
        var projectionResults = await engine.ExecuteToListAsync(
            "echo \"{\\\"name\\\":\\\"alpha\\\",\\\"size\\\":1}\" | from-json | pick { name, size } | rename name Name size Bytes");

        Assert.Collection(variableResults, item => Assert.Equal("hello", item));

        var projection = Assert.IsType<ProjectedObject>(Assert.Single(projectionResults));
    ...
    public async Task Take_while_skip_while_and_tee_can_shape_and_capture_pipelines()
    {
        ...
        var takeResults = await engine.ExecuteToListAsync("echo 1 2 3 1 | take-while $it < 3");
        var skipResults = await engine.ExecuteToListAsync("echo 1 2 3 1 | skip-while $it < 3");
    ...
    public async Task Text_shaping_commands_work_together()
    {
        ...
        var hashProjection = Assert.IsType<ProjectedObject>(Assert.Single(hashResults));
    ...
    public async Task File_system_helper_commands_work()
    {
        ...
        Assert.Equal(true, Assert.IsType<ProjectedObject>(Assert.Single(existsResults))["Exists"]);
        Assert.Equal(true, Assert.IsType<ProjectedObject>(Assert.Single(fileResults))["IsFile"]);
        Assert.Equal(true, Assert.IsType<ProjectedObject>(Assert.Single(dirResults))["IsDirectory"]);
    ...
    public async Task Clr_cast_and_reflection_commands_work()
    {
        ...
        var memberResults = await engine.ExecuteToListAsync("members string | where Name == Length | first | get Kind");
        var methodResults = await engine.ExecuteToListAsync("methods string | where Name == Contains | first | get ReturnType");
        var constructorResults = await engine.ExecuteToListAsync("constructors System.Text.StringBuilder | count");
        var catalogResults = await engine.ExecuteToListAsync("types System.String | where FullName == System.String | first | get Name");
    ...
    [Fact]
    public async Task Shell_state_commands_manage_aliases_functions_environment_and_history()
    {
        var runtime = ToshRuntime.CreateDefault();
        var engine = new ToshEngine(runtime);
        var variableName = $"TOSH_TEST_{Guid.NewGuid():N}";

        await engine.ExecuteToListAsync("alias tosh_alias = echo hi");
        await engine.ExecuteToListAsync("def tosh_func() { echo hi }");

        var unaliasResults = await engine.ExecuteToListAsync("unalias tosh_alias");
        var undefResults = await engine.ExecuteToListAsync("undef tosh_func");
        await engine.ExecuteToListAsync($"export {variableName} toast");
        var unsetResults = await engine.ExecuteToListAsync($"unset {variableName}");
        ...
        Assert.Equal(true, Assert.IsType<ProjectedObject>(Assert.Single(unaliasResults))["Removed"]);
        Assert.Equal(true, Assert.IsType<ProjectedObject>(Assert.Single(undefResults))["Removed"]);
        Assert.Null(Environment.GetEnvironmentVariable(variableName));
        Assert.False(Assert.IsType<EnvironmentVariableEntry>(Assert.Single(unsetResults)).IsSet);
